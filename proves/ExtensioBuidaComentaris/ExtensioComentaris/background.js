...
async function scriptPerInjectar(nouText) {
  ...
  let botons = Array.from(document.querySelectorAll('a[data-ng-click*="showCommentsContingutsModal"]'))
    .filter(boto => !boto.classList.contains('emptyIcon') && !boto.hasAttribute('disabled') && boto.getAttribute('data-ng-disabled') !== 'true');
  if(botons.length === 0) {
    console.log("Provo a recuperar els de grup i matèria.");
    botons = Array.from(document.querySelectorAll('a[data-ng-click*="showDialogMoreOptions"]'))
  ...
  for (let i = 0; i < botons.length; i++) {
    ...
    if(!tipusPaginaGrupMateriaa)  {
      const textarea = document.querySelector('textarea[data-ng-model="modalComentaris.comentaris"]');
      const botoDesa = document.querySelector('a[data-ng-click*="saveComentariContingut"]');
      ...
      const textarea = document.querySelector('textarea[data-ng-model="commentsToModify.commentsToModifyModal"]');
